...
namespace json
{
  ...
  struct Value
  {
    ...
    virtual Value* goToKey(std::string s) const {
      Value* temp;
      return temp;
    }

    //Gets the acutal string from a string object
    ...
    virtual int getSizeArr() {
      int q = 0;
      return q;
    }

    virtual std::vector<Value*> getArray(){
      std::vector<Value*> newVal;
      return newVal;
    }
  ...
  struct Object : Value, std::unordered_map<std::string, Value*>
  {
    ...
    Value* goToKey(std::string s) const override{
